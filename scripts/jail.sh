...
update_motd="NO"
dumpdev="NO"
hostname="builder.deiter.local"
ifconfig_builder1="inet 172.27.10.22/24"
defaultrouter="172.27.10.1"
EOF

cat >etc/hosts <<EOF
127.0.0.1	localhost.deiter.local	localhost
172.27.10.22	builder.deiter.local	builder
EOF
...
kadmin -l ext_keytab --keytab=etc/krb5.keytab host/builder.deiter.local
...
_list=$(awk '/^\//{print $1}' /etc/fstab.builder | sort)
for _fs in $_list; do
